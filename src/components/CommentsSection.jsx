...
export default function CommentsSection({ songId }) {
  ...
  const postComment = async e => {
    ...
    setPosting(true);
    try {
      const res = await fetch(`http://localhost:5000/api/comments/${songId}`, {
        method: "POST",
    ...
    <div className="mt-8 bg-white/80 dark:bg-black/80 rounded-xl shadow-lg p-6">
      ...
      {loading ? <div>Loading...</div> : (
        <div className="flex flex-col gap-3">
          ...
          {comments.map(com => (
            <div key={com._id} className="bg-gray-50 rounded px-4 py-2 flex items-start gap-2">
              <div className="flex-1">
                <div className="text-sm font-bold">{com.user}</div>
                <div className="text-gray-800 text-sm">{com.text}</div>
              </div>
              {user && com.user === user.email && (
                <button className="text-xs text-red-500 ml-2" onClick={() => deleteComment(com._id)}>Delete</button>
              )}
            </div>
          ))}
        </div>
